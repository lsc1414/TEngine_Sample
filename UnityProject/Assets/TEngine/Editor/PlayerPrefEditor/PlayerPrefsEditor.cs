...
public class PlayerPrefsEditor : EditorWindow
{
    ...
    private void DrawBottomMenu()
    {
        ...
        if (GUILayout.Button("加载存档"))
        {

            string filename = EditorUtility.OpenFilePanelWithFilters("请选择生成的json目录", Application.dataPath, new string[] { "json", "json" });
            // Debug.Log(filename);
            System.Text.StringBuilder result = new System.Text.StringBuilder("");
            try
            {
                var result2 = File.ReadAllText(filename);
                var importedPairs = Utility.Json.ToObject<List<PlayerPrefPair>>(result2);
                if (importedPairs == null) return;
                ...
                // Force a save
                PlayerPrefs.Save();
                EditorUtility.DisplayDialog("加载成功", "加载成功", "ok");
            }
            catch (Exception e)
            {
                Debug.LogError(e);
        ...
        if (GUILayout.Button("保存到json文件"))
        {
            var prefPairs = new List<PlayerPrefPair>(RetrieveSavedPrefs(PlayerSettings.companyName, PlayerSettings.productName));
            var jsonStr = Utility.Json.ToJson(prefPairs);
            Debug.Log(jsonStr);
            if (!String.IsNullOrEmpty(jsonStr))
            {
                var temp = EditorUtility.OpenFolderPanel("请选择生成的json目录", Application.dataPath, "");
                if (!String.IsNullOrEmpty(temp))
                {
                    //resultFolder = temp;
                    using (FileStream we = new FileStream($"{temp}\\{PlayerSettings.productName}_{DateTime.Now.ToString("yyyyMMdd_HHmmss")}.json", FileMode.OpenOrCreate, FileAccess.Write))
                    {
                        byte[] by = System.Text.Encoding.Default.GetBytes(jsonStr.ToString());
